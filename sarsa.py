...
"""
SARSA is similar to Q-learning.
The key difference between SARSA and Q-learning is that SARSA is an on-policy algorithm.
It implies that SARSA learns the Q-value based on the action performed by the current policy instead of the greedy policy.
"""
...
def egreedy_policy(grid, intended_action, epsilon):
    other_actions = list(grid.action)
...
def main(is_sarsa, episode_limit):
    ...
    Q_values = initialize_Q_values(all_states, grid.action, grid.trapstate, grid.goalstate)
    ...
    while episode < episode_limit:
        ...
        # ------ Finish Episode ------
        if cur_state == grid.goalstate or cur_state in grid.trapstate:
        ...
        if next_state == grid.goalstate or next_state in grid.trapstate:
    ...
    for y in range(0, grid.height):
        ...
        for x in range(0, grid.width):
            ...
            if (x, y) in grid.walls:
                ...
                if (x, y) == grid.goalstate or (x, y) in grid.trapstate:
                    Q = Q_values[(x, y)]
                    print("%.2f\t| " % Q, end="")
                else:
...
plt.plot(list(range(0, episode_sarsa)), episode_time_sarsa)
