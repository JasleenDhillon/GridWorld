...
import cv2
"""
GridWorldMDP class has reward grid, value iteration, policy iteration, and generate experience modules.
Value Iteration Algorithm.
    ...
    Returns:
        A tuple (policy, V) of the optimal policy and the optimal value function along with the grid and graph
...
class GridWorldMDP:
    ...
    _num_of_actions = len(_directions)
...
#used to run policy iterations
    def run_policy_iterations(self, discount=1.0,
                              iterations=10):
        utility_grids, policy_grids = self._init_utility_policy_storage(iterations)

        policy_grid = np.random.randint(0, self._num_of_actions,
                                        self.shape)
...
#converts indices of grids to coordinates
    def grid_indices_to_coordinates(self, indices=None):
        if indices is None:
            indices = np.arange(self.size)
        return np.unravel_index(indices, self.shape)
#converts coordinates of grid to indices
    ...
    def _create_transition_matrix(self,
        ...
        T = np.zeros((M, N, self._num_of_actions, M, N))

        r0, c0 = self.grid_indices_to_coordinates()

        T[r0, c0, :, r0, c0] += no_action_probability

        for action in range(self._num_of_actions):
            for offset, P in action_probabilities:
                direction = (action + offset) % self._num_of_actions
...
#calculates utility by using discount factor and utility grid
...
#used to plot the direction in which the action must take place
# directions are used by <, >, v, ^
#filled the color with white
...
# Used to know the time of convergence with the help of graph
...
# grid shape, terminal, initial, traps, obstacles are intialised
# goal and trap rewards are initialised

if __name__ == '__main__':
    shape = (8, 8) #no of rows and columns in the grid
    goal = (0, 7) #terminal state
    trap = [(0, 4), (0, 0), (3, 2), (5, 6)]
    obstacle = [(0, 1), (0, 5), (0, 6), (2, 2), (2, 3), (2, 4), (3, 4), (3, 6), (4, 1), (4, 2), (4, 6), (5, 2), (5, 5), (6, 5), (6, 6), (7 , 3)]
